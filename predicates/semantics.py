...
class Model(Generic[T]):
    ...
    def evaluate_formula(self, formula: Formula,
        ...
        if is_relation(formula.root):
            return tuple(self.evaluate_term(argument, assignment)
                         for argument in formula.arguments) in \
    ...
    def is_model_of(self, formulas: AbstractSet[Formula]) -> bool:
        ...
        for formula in formulas:
            ...
            def check_all_assignments(index: int, assignment: Mapping[str, T]) -> bool:
                ...
                for value in self.universe:
                    if not check_all_assignments(
                            index + 1, {**assignment, variable: value}):
                        return False
                return True
